...
$(document).ready(function() {
  ...
  var getNumber = function () {
    $.get('/api/', num => { // 获取服务端的随机数
      if ($(this).find(".unread")[0]) {
        ...
        this.HadClicked = true; // 增加属性：其他气泡不可点击
  ...
  addElement();
  // 模拟同时按下A~E按钮，并在所有随机数到达后，立即按下大气泡的过程。
  var FiveFigure = function () {
    $('.apb')[0].onclick = function() {
      var lis = $("#control-ring li");
      for (var i = 0; i < lis.length; i++) {
        lis[i].click();
      }
      var time = setInterval(function () {  //设置间隔模拟点击
        if ($("#info-bar")[0].Click) {
          setTimeout($("#info-bar")[0].click(), 200);
          clearInterval(time);
        }
      }, 500);
    }
