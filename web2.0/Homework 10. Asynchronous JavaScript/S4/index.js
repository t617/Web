...
$(document).ready(function() {
  ...
  var getNumber = function () {
    $.get('/api/', num => { // 获取服务端的随机数num
      if ($(this).find(".unread")[0]) { //找到刚生成的.unread读入数字
        var all_flag = true;
  ...
  var To_Blue = function () {
    ClickBuble_flag = false; //其他设为不可点击
    $("#control-ring li").each(function () {
      if (!$(this)[0].HadClicked) {
  ...
  // 随机数数组
  var Random_Array = function(min, max, sum) {
    var arr = [];
    for(var i = 0; i < sum; i++) {
  ...
  var AddResult = function(array) { //用于显示A—E
    var result = document.createElement("span");
    result.className = "sequence";
    for (var i = 0; i < array.length; i++) {
      result.innerHTML += String.fromCharCode('A'.charCodeAt(0) + array[i]);
    }
    $("#info-bar")[0].append(result);
  }
  ...
  addElement();
  /* 乱点鸳鸯 获取随机数然后按税基数顺序get得num*/
  var InOrder = function() {
    $('.apb')[0].onclick = function() {//点击@时
      var array = Random_Array(0, 4, 5);
      AddResult(array);
